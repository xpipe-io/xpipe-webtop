...
You can run the container locally via docker compose by utilizing the files in `./compose`. First, you have to set the `XPIPE_DATA_DIR` location in the `.env.local` file to the mounted user data directory for the container.

For the usage of `XPIPE_REPO_DIR` in the env file, see further down below. You can leave this value empty.
...
You can make use of a local development within the container by cloning the repository with `git clone https://github.com/xpipe-io/xpipe` and then passing a bind mount to `<path to repo dir>:/config/xpipe-dev`. You can do this by setting `XPIPE_REPO_DIR` in the `.env.local` file. This will then trigger a gradle build within the container.
